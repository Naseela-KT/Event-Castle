...
const Message = ({message,own,setIsUpdated}) => {
 ...
 const handleOpenLeft = (msgId:string) => {
  ...
  const handleDeleteForMe=async(side:string)=>{
    axiosInstanceVendor
    .patch("/delete-for-me",{msgId:messageIdToDelete,id:vendor?._id},{withCredentials:true})
    .then((response) => {
      console.log(response);
      setIsUpdated(true)
      if(side=="right"){
        ...
        <div className="flex items-end justify-end">
          <div className="flex flex-col space-y-2 text-xs mfx-w-xs mx-2 order-2 items-end">
            <div>
              ...
              {message?.deletedIds.includes(vendor?._id)?"":<p className="flex items-end justify-end text-xs text-gray-500 mr-2">
        ...
        <div className="flex items-end">
          <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-start">
            ...
            {message?.isDeleted ? (
              ...
              </span>):message?.deletedIds.includes(vendor?._id)?"":(<span className="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-200 text-gray-600" onClick={()=>handleOpenLeft(message?._id)}>
                {message.text}
              </span>)}
            </div>
          </div>
